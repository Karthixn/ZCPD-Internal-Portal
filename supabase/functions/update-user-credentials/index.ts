...
serve(async (req) => {
  ...
  try {
    ...
    const { data: { user: caller }, error: callerErr } = await supabaseAdmin.auth.getUser(authHeader.replace("Bearer ", ""));
    ...
    if (callerProfile?.role !== "ftc") return new Response(JSON.stringify({ error: "Only FTC can update credentials" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (!email && !password) return new Response(JSON.stringify({ error: "Provide at least email or password" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (email) await supabaseAdmin.from("profiles").update({ email }).eq("id", user_id);
